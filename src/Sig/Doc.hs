...
import Sig.Types
       (Config(..), FingerprintSample(FingerprintSample), Signer(..),
        Mapping(..), Signature, Archive(..))
...
instance ToDoc Archive where
    toDoc Archive{..} =
        text "Mappings:" <$>
        hang 2 (indent 2 (toDoc archiveMappings)) <> text "Signatures:" <$>
...
instance ToDoc (Map Text Mapping) where
    toDoc x =
        vsep
            (map
                 (\(a,b) ->
                       toDoc a <$> hang 2 (indent 2 (toDoc b)))
                 (M.toList x)) <>
        linebreak
...
instance ToDoc PackageIdentifier where
    toDoc x =
        toDoc (pkgName x) <> text "-" <>
        text (intercalate "." (map show (versionBranch $ pkgVersion x))) <>
        case versionTags (pkgVersion x) of
            [] -> empty
            xs -> text "-" <> text (intercalate "-" xs)
...
putPkgOK
    ...
    => PackageIdentifier -> m ()
putPkgOK pkg =
    liftIO (putDoc (text "  " <> fill 74 (toDoc pkg) <> "OK" <> linebreak))
