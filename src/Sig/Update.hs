...
import Sig.Types
       (SigException(ArchiveUpdateException, SigServiceException))
...
update url = do
    ...
    request <- parseUrl (url <> "/download/archive")
    catch
        (withManager
             (\mgr ->
                   do res <- http request mgr
                      (responseBody res) C.$$+- sinkFile tempFile))
        (\e ->
    ...
    when
        ...
        (do time <- getCurrentTime
            renameDirectory
                archivePath
                (formatTime defaultTimeLocale (archivePath <> "-%s") time))
    (code,_out,err) <-
        readProcessWithExitCode "tar" ["xf", tempFile, "-C", configPath] mempty
    unless (code == ExitSuccess) (throwIO (ArchiveUpdateException err))
