...
import Sig.Types
       (SigException(GPGSignException),
        FingerprintSample(fingerprintSample), Signature(Signature))
import System.Directory
       (getTemporaryDirectory, getDirectoryContents,
...
sign url filePath = do
    ...
    (_code,_out,_err) <-
        readProcessWithExitCode
            ...
            mempty
    cabalFiles <-
        (filter (isSuffixOf ".cabal")) <$> (getDirectoryContents workDir)
    if length cabalFiles < 1
        then undefined
        else do
            pkg <- cabalFilePackageId (workDir </> head cabalFiles)
